...
class ProductsDB:
    ...
    def insert_product(self, product):
        ...
        for index, row in product.iterrows():
            ...
            existing_product = pd.read_sql_query(query, self.conn, params=params)
            # row['EffectiveFrom'] = datetime.today().strftime('%Y-%m-%d')
            # row['EffectiveTo'] = '9999-12-31'
            # row['CurrentFlag'] = 1
            if not existing_product.empty:
                print(f"Product {params} already exists in database")
                ...
                row.to_sql('products', self.conn, if_exists='append', index=False)
                self.conn.commit()
            else:
